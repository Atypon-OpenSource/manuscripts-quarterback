...
const createControl = (change: TrackedChange, action: string, xLink: string) => {
  return `
...
const getClassnames = (...types: Array<CLASSES | null>): string[] => {
  return [BASE_CLASS].concat(types.filter(Boolean).map((type) => `${BASE_CLASS}--${type}`))
}

const addClassnamesToEl = (el: HTMLElement, ...types: Array<CLASSES | null>) => {
...
const decorateChanges = (state: EditorState, showButtons?: boolean): Decoration[] => {
