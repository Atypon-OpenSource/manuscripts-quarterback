...
import { HistoryItem, isReview, isSnapshot } from 'stores/HistoryStore'
...
const HistoryList = observer((props: IProps) => {
  const { className, viewProvider } = props
  const {
    authStore: { isAdmin, user },
    documentStore: { inspectedSnapshot, resumeEditing },
  ...
  function handleResumeEditing() {
    resumeEditing()
    const { currentDocument } = stores.documentStore
