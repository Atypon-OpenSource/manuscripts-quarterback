...
describe('Track changes', () => {
  ...
  it('Should mark typed text as deleted', () => {
    ...
    cy.get('button[aria-label="toggle-track-changes"]').click()
    cy.get('#example-editor').focus().type('{backspace}{backspace}{backspace}{backspace}')
  ...
  it('Should fix marks inserted with incomplete track attributes', () => {
    ...
    cy.window().then(window => {
      ...
      const trackPlugin = view.state.plugins.find(p => p.key === 'track-changes$')
    ...
    cy.window().then(window => {
      ...
      tr.addMark(1, 5, view.state.schema.marks.tracked_insert.create({
        dataTracked: {},
        pending_bg: 'yellow',
      }))
      view.dispatch(tr)
