...
import { DeleteAttrs, InsertAttrs, UserData } from './types'
...
function recurseContent(
  ...
  if (node.isInline) {
    return markInlineNodeChange(node, insertAttrs, userColors)
  } else if (node.isBlock) {
    const updatedChildren: PMNode[] = []
    node.content.forEach((child) => {
      updatedChildren.push(recurseContent(child, insertAttrs, userColors))
    })
...
function setFragmentAsInserted(
  ...
  inserted.forEach((n) => {
    updatedInserted.push(recurseContent(n, insertAttrs, userColors))
  })
  ...
  doc.nodesBetween(from, to, (node, pos) => {
    if (node.isInline) {
      ...
      if (firstInlineNode) {
        ...
        leftMarks = shouldMergeMarks(leftNode, addedAttrs)
      }
      if (lastInlineNode) {
        rightNode = doc.resolve(Math.min(pos + node.nodeSize, to)).nodeAfter
        rightMarks = shouldMergeMarks(rightNode, addedAttrs)
      }
...
function deleteInlineIfInserted(
  ...
  if (node.marks.find((m) => m.type === schema.marks.tracked_insert)) {
    ...
    const leftMarks = shouldMergeMarks(leftNode, deleteAttrs)
    const rightNode = newTr.doc.resolve(end).nodeAfter
    const rightMarks = shouldMergeMarks(rightNode, deleteAttrs)
    const fromStartOfMark =
        ...
        .forEach((fromA: number, toA: number, fromB: number, toB: number) => {
          ...
          if (newSliceContent.size > 0) {
            ...
            const insertedSlice = new Slice(
              setFragmentAsInserted(newSliceContent, insertAttrs, userData),
              openStart,
              openEnd
            )
